            ...
            <div class="col-md-12 grid-margin stretch-card">
                <div class="card">
                    <div class="card-body">
                        ...
                        <hr>
                        <form action="POST" id="editArticleForm" class="col-12 col-xl-8">
                            <input type="hidden" name="slug" id="slug" value="{{ $data['article']['slug'] }}">
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="title"
                                    value="{{ $data['article']['title'] }}" autocomplete="off" required>
                                <label for="title">Judul</label>
                            </div>
                            <div class="form-floating mb-3">
                                <input type="text" class="form-control" id="keyword" placeholder="Kata kunci artikel"
                                    autocomplete="off" value="{{ $data['article']['keyword'] }}" required>
                                ...
                                <select class="form-select" id="type" required>
                                    ...
                                    @foreach ($data['types'] as $type)
                                        <option value="{{ $type['id'] }}"
                                            {{ $data['article']['article_type_id'] == $type['id'] ? 'selected' : '' }}>
                                            {{ $type['name'] }}</option>
                                    @endforeach
                                </select>
                                ...
                                <select class="form-select" id="category" required>
                                    ...
                                    @foreach ($data['categories'] as $category)
                                        <option value="{{ $category['id'] }}"
                                            {{ $data['article']['category_id'] == $category['id'] ? 'selected' : '' }}>
                            ...
                            <div class="mb-3">
                                ...
                                <div id="previewThumbnailWrapper" style="width: 100%; max-height: 1080px"
                                    class="border border-primary rounded border-opacity-25 overflow-hidden text-center my-3">
                                    <img src="{{ $data['article']['img_thumbnail'] }}" alt="Preview Thumbnail"
                                        class="img-fluid" id="previewThumbnail" style="height: 100%;">
                                </div>
                            </div>
                            <div class="mb-3">
                            ...
                            <div class="form-floating mb-3">
                                <textarea name="excerpt" id="excerpt" class="form-control" style="height: 100px" required autocomplete="off"
                                    maxlength="160">{!! $data['article']['excerpt'] !!}</textarea>
                            ...
                            <div class="mb-3 d-flex justify-content-end gap-2">
                                ...
                                <button class="btn btn-warning" type="button" data-draft="true"
                                    onclick="editArticle(this)">
                                    Draft
                                </button>
                            </div>
    ...
    <script>
        ...
        const editArticle = (element) => {
            ...
            const data = {
                ...
                'article_type_id': $('#editArticleForm #type').find(':selected').val(),
                'lang_id': $('#editArticleForm #language').val(),
                'img_thumbnail': $('#editArticleForm #thumbnail')[0].files[0],
                'is_draft': draft,
                'excerpt': $('#editArticleForm #excerpt').val(),
                'keyword': $('#editArticleForm #keyword').val(),
                'body': bodyContent,
                'slug': slug
            };
            const formData = new FormData();
            ...
            $.ajax({
                ...
                error: (xhr, status) => {
                    ...
                    if (xhr.status == 422) {
                        return toast('warning',
                            'Ensure each field has the correct content'
                        );
                    }
